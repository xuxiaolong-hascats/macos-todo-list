...
Double-click a todo title to edit it inline. Press Return or click the checkmark to save, or press Escape to cancel. Use the gear button in the todo panel to adjust panel opacity from `0` to `1` and choose a theme color. The footer can clear completed todos. If the panel is set fully transparent, use `Reset Appearance` from the menu bar item.
